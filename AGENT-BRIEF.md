...
- **Découpage en sous-tranches (un commit validé chacune)** : 2.0 deps + factory modèle par-rôle · 2.1 router parent (classify → arêtes → sous-graphes stub) · 2.2 seam streaming bout-en-bout + front `useChat` · 2.3 `revise` 1er tour · 2.4 multi-tours + `evaluate` gated + `PostgresSaver` · 2.5 update mémoire incrémental + `session_trace` · 2.6 resolver leçon (candidats + `clarify`) + finitions end-to-end · **2.7 tier temporel** (migration `last_reviewed_at` + bump applier découplé + 3e tier de sélection récence/intervalle).
