...
4. ✅ **[FAIT — étape 4, détail en §18]** **Langfuse** (dernière étape) : plateforme intégrée et **trois aspects testés en passant par elle** — **observabilité** (traces nœuds/graphe via OTEL + CallbackHandler), **coût** (auto-calculé, par-rôle via le nom de modèle par nœud, par séance via `sessionId`), **eval** (2 LLM-as-judge factuel + socratique calibré `precision_bar`, en Dataset Run sur cas dorés ; discrimination prouvée). **Fin du build.**
...
## 18. Étape 4 — Langfuse (dernière étape) — journal

> **Cadrage acté avec l'utilisateur (16/06) :** **Langfuse Cloud** (pas self-hosting — la stack v3+ exige Postgres+Clickhouse+Redis+Minio, lourde pour un POC ; clés `pk`/`sk` dans `.env`, gitignoré). Découpage : **4.1 obs+coût d'abord**, **4.2 eval ensuite** (un commit chacun).

### 18.1 Observabilité + coût — ✅
- **SDK v5 OTEL-based** (`@langfuse/otel` `LangfuseSpanProcessor` + `@langfuse/langchain` `CallbackHandler` + `@opentelemetry/sdk-node`). Le `CallbackHandler` v5 est un pur émetteur de spans OTEL ; **le flush vit sur le span processor**, pas sur le handler (≠ doc v4).
- **Câblage** ([observability/langfuse.ts](apps/backend/src/observability/langfuse.ts)) : `startObservability()` démarre un `NodeSDK` au boot (no-op sans les 2 clés → app + tests inchangés) ; `createTraceHandler(threadId)` → un handler par tour, `sessionId = thread_id` (= une séance) ; passé via `callbacks` au `router.stream` → **chaque nœud + appel modèle = span/generation imbriqué, gratis** (propagation des callbacks LangChain). `flushObservability()` (`forceFlush`) dans un `finally` du handler de stream → traces visibles immédiatement.
- **Coût auto-calculé** : Langfuse dérive le $ de `(nom de modèle + usage tokens)` — les IDs Anthropic (`claude-sonnet-4-6`, `claude-haiku-4-5-*`) **sont dans sa table de prix** (vérifié, coût non-nul). Angle **par-rôle** visible via le nom de modèle par nœud (Haiku sur classify/resolveLesson, Sonnet sur socratic). Angle **par séance** = groupé par `sessionId`.
- **Vérifié live** : 1 tour `revise` → trace `LangGraph` (session, $0.0084, 13 observations : spans classify/resolveLesson/revise/socratic + 3 generations par-modèle), confirmée via l'API publique Langfuse. Aucun warning d'export. Garde-fous verts (typecheck, lint, 127 tests).

### 18.2 Eval (LLM-as-judge factuel + socratique) — ✅
- **Approche actée (16/06) :** juges en code + **Langfuse Dataset Run**, sur **cas dorés curés** (pas évaluateurs UI managés, pas dérivé de `*_history` — décision utilisateur).
- **SDK** : `@langfuse/client` `experiment.run` / `dataset.runExperiment({ task, evaluators })` — crée le dataset run, trace chaque item, attache les scores. ⚠️ **Bugs de binding `this` dans 5.4.1** : `langfuse.createDataset`/`createDatasetItem`/`getDataset` sont des alias non-bindés (les `prompt.*` le sont, pas les `dataset.*`) → passer par `langfuse.api.datasets.create` / `langfuse.api.datasetItems.create` / `langfuse.dataset.get`.
- **Câblage** ([eval/](apps/backend/src/eval/)) : `cases.ts` = 12 cas dorés (3 leçons CM2 × les 3 `precision_bar`) + `caseToBundle` (bundle synthétique → le **vrai** `buildSocraticSystem` tourne **sans DB**) ; `judges.ts` = 2 juges (functional core : `buildFactualJudgeSystem` calibré par `precision_bar` + `buildSocraticJudgeSystem`, `scoreFactual`/`scoreSocratic` purs ; shell = `bindTools` §16.4 §1) ; `run-eval.ts` = upsert dataset → `dataset.runExperiment` (task = tuteur socratique réel ou sortie de contrôle ; evaluators = les 2 juges). Lancer : `pnpm --filter @tuteur/backend eval`.
- **Discrimination prouvée** (l'éval peut échouer sur un vrai bug, CLAUDE.md) : 3 cas de **contrôle** plantent **une** faute chacun (2 « réponse révélée » → juge socratique ; 1 « fait faux » → juge factuel). Run live : factual **0.917** (11/12), socratic **0.833** (10/12) ; per-item, les juges attrapent **exactement** les 3 contrôles, les 9 cas live (vrai tuteur) scorent 1/1. Rôle `judge` du factory réutilisé. Garde-fous verts (typecheck, lint, 141 tests dont 14 d'éval).
- **Différé (hors POC)** : light-judge de sanity sur l'extraction d'ingestion (brief §17.1) ; eval branchée sur `*_history`.

### 18.3 État — étape 4 terminée ✅ → **fin du build** (§12.4, §13). Trois aspects testés via Langfuse : observabilité (18.1), coût (18.1, auto + par-rôle + par séance), eval (18.2).
