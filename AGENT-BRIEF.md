...
## 16. Étape 2 — journal d'implémentation & état (mise à jour 14/06, post-2.4)
...
- **`hydrate`/`socratic`/`evaluate`/`finish`** : résolvent élève+leçon par **scaffolding temporaire** (`findFirstOrThrow orderBy createdAt asc` — **à remplacer en 2.6**). `hydrate` (1er tour) arme la file via `selectConcepts` ; `socratic` & `evaluate` re-hydratent par tour (`loadCurrentConcept`, DB autoritaire) et lisent le concept au curseur.
...
- **Pas encore câblés** : écriture mémoire (`advance` n'écrit pas encore — 2.5), resolver de leçon (2.6), tier temporel (2.7).

### 16.4 Leçons apprises / pièges (À LIRE avant 2.5+)
