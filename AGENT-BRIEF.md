...
## 16. Étape 2 — journal d'implémentation & état (mise à jour 14/06, post-2.5)
...
- **Checkpointer** : `PostgresSaver.fromConnString(DATABASE_URL)` + `.setup()` au boot (idempotent), passé à `buildRouterGraph(checkpointer)` → `.compile({checkpointer})`. Sur le graphe parent uniquement (`checkpoint/index.ts`). Tests : in-memory.
- **Graphe parent** (`RouterState` = `MessagesAnnotation` + scalaires `intent`/`confidence` + canaux de session revise `studentId`/`lessonId`/`sessionConceptIds`/`conceptCursor`/`turnsOnConcept`/`reviseActive`/`masterySignal`, tous reducer last-value) :
  - `START → routeStart` : `reviseActive ? evaluate : classify` (saut de `classify` sur flow actif, §5.1).
...
- **Pas encore câblés** : resolver de leçon (2.6), tier temporel `last_reviewed_at` (2.7).
...
11. **Migration Prisma + checkpointer = `migrate dev` veut RESET (⚠️ destructif).** Le `PostgresSaver` crée ses tables (`checkpoints*`) dans le **même schéma `public`** que le domaine → Prisma les voit comme une **drift** et propose de **drop la base**. **Ne JAMAIS `migrate dev` sur la base de dev.** À la place : écrire le dossier de migration **à la main** (`prisma/migrations/<timestamp>_nom/migration.sql`, SQL brut) puis `prisma migrate deploy` (pas de détection de drift, pas de reset) + `prisma generate`. Les tests appliquent déjà `migrate deploy` (base `tuteur_test`, sans tables checkpointer). **À refaire tel quel pour la colonne `last_reviewed_at` de 2.7.**
